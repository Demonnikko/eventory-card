  ...
  plugins: [
    VitePWA({
      ...
      workbox: {
        ...
        // Что НЕ отдаём из precache:
        //   /api/    — живые серверные функции (публикация/чтение визитки);
        //   /v/, /r/ — публичная визитка и запись отзыва. Их открывают
        //     посторонние люди в обычном браузере; им нужна свежая страница
        //     с сервера (og-теги, актуальные данные, последний CSS), а не
        //     закэшенный у владельца index. Иначе клиент видит устаревшую
        //     вёрстку, которую он никогда не «обновит» — кнопки у него нет.
        navigateFallbackDenylist: [/^\/api\//, /^\/v\//, /^\/r\//],
      ...
      manifest: {
        ...
        shortcuts: [
          {
            // Долгий тап по иконке приложения → сразу красивая визитка с QR
            // (экран present), минуя онбординг, редактор и вкладку «Поделиться».
            // Самый быстрый способ показать себя клиенту, не заходя внутрь.
            name: 'Показать визитку',
            short_name: 'Визитка',
            description: 'Открыть визитку с QR-кодом для клиента',
            url: '/#/present'
          }
        ]
      }
